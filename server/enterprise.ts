...
// ============================================================================
// PHASE 10 ENTERPRISE DATABASE INITIALIZATION (AUTO-MIGRATION)
// ============================================================================
(async () => {
  ...
  const safeCreateTable = async (tableName: string, createDdl: string) => {
    try {
      const exists = await checkTableExists(tableName);
      if (!exists) {
        await db.execute(sql.raw(createDdl));
        console.log(`[Phase 10 Server] Created table: ${tableName}`);
      } else {
        console.log(`[Phase 10 Server] Table already exists: ${tableName}`);
      }
    } catch (err: any) {
      console.warn(`[Phase 10 Server] Warning: Could not verify or create table ${tableName}. Reason: ${err.message}`);
    }
  };
  ...
  try {
    ...
    await safeCreateTable("workflow_approvals", `
    ...
    // 3. Scheduler jobs
    await safeCreateTable("scheduler_jobs", `
    ...
    // 4. API keys
    await safeCreateTable("api_keys", `
    ...
    // 5. Webhooks
    await safeCreateTable("webhooks", `
    ...
    // 6. Webhook logs
    await safeCreateTable("webhook_logs", `
    ...
    // 7. Saved reports
    await safeCreateTable("saved_reports", `
    ...
    // Seed default background worker cron schedules
    try {
      await db.execute(sql.raw(`
        INSERT INTO scheduler_jobs (name, cron_expression, job_type, payload_json, status, is_active)
        VALUES 
          ('Nightly Database Backup', '0 2 * * *', 'Backup', '{}'::jsonb, 'Idle', true),
          ('Automated Monthly Payroll Run', '0 0 28 * *', 'Payroll', '{}'::jsonb, 'Idle', true),
          ('Daily Expiry Documents Audit', '0 6 * * *', 'Report', '{}'::jsonb, 'Idle', true),
          ('Fleet Fuel Forecast Scheduler', '0 4 * * *', 'Maintenance', '{}'::jsonb, 'Idle', true)
        ON CONFLICT (name) DO NOTHING;
      `));
      console.log("[Phase 10 Server] Seeded default background worker cron schedules.");
    } catch (err: any) {
      console.warn("[Phase 10 Server] Seeding scheduler_jobs warning:", err.message);
    }
    ...
    try {
      await db.execute(sql.raw(`
        ...
        VALUES
          ('Trip Dispatch Pre-Approval', 'Trip', 'Standard multi-level approval before routing high-value containers', '[{"step": 1, "role": "Operations Manager", "required": true}, {"step": 2, "role": "Super Admin", "required": true}]'::jsonb, true),
          ('Monthly Payroll Disbursement Approval', 'Payroll', 'Double-signature required by Accounts and HR before disbursement', '[{"step": 1, "role": "Accountant", "required": true}, {"step": 2, "role": "HR Manager", "required": true}]'::jsonb, true)
        ON CONFLICT (name) DO NOTHING;
      `));
      console.log("[Phase 10 Server] Seeded default system workflows.");
    } catch (err: any) {
      console.warn("[Phase 10 Server] Seeding workflows warning:", err.message);
    }

    console.log("[Phase 10 Server] Database tables checking, auto-migration and default seeding completed successfully.");
  } catch (error) {
    console.error("[Phase 10 Server] Error setting up database schemas:", error);
  }
})();
