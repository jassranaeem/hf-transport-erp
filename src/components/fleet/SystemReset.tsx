...
export default function SystemReset({
  ...
  if (!isSuperAdmin) {
    return (
      <div className="rounded-xl border border-[#FECACA] bg-[#FEF2F2] p-4 text-sm text-[#B91C1C] flex items-center gap-2">
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      ...
      <div className="rounded-xl border border-[#E5E7EB] bg-white p-4 space-y-2">
        <h3 className="text-sm font-semibold flex items-center gap-1.5"><Save className="w-4 h-4 text-[#16A34A]" /> Take a backup</h3>
        ...
        <button
          onClick={takeBackup}
          disabled={backingUp}
          className="flex items-center gap-1.5 bg-[#16A34A] text-white text-sm font-semibold rounded-lg px-4 py-2 disabled:opacity-60"
        >
      ...
      <div className="rounded-xl border border-[#E5E7EB] bg-white overflow-hidden">
        <div className="px-3 py-2 text-xs font-bold bg-[#F3F7F4]">Recent backups</div>
            ...
            {backups.map((b) => (
              <tr key={b.id} className="border-t border-[#F3F4F6]">
                ...
                <td className="px-2 py-1.5">
                  {b.restorable ? (
                    <span className="text-[10px] font-bold rounded px-1.5 py-0.5 bg-[#DCFCE7] text-[#15803D]">Restorable</span>
                  ) : (
                ...
                <td className="px-2 py-1.5 text-right">
                  {b.restorable && (
                    <button
                      ...
                      className="text-[11px] font-semibold text-[#B45309] hover:underline flex items-center gap-1 ml-auto"
                    >
      ...
      {restoreTarget && (
        <div className="rounded-xl border border-[#FDE68A] bg-[#FFFBEB] p-4 space-y-2">
          ...
          <p className="text-[12px] text-[#92400E]" dir="auto">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="border border-[#FDE68A] rounded px-2 py-1.5 text-sm font-mono flex-1"
              dir="ltr"
            />
            <button
              onClick={doRestore}
              disabled={restoreText !== RESTORE_PHRASE || restoring}
              className="flex items-center gap-1.5 bg-[#B45309] text-white text-sm font-semibold rounded-lg px-4 py-2 disabled:opacity-40"
            >
      ...
      <div className="rounded-xl border-2 border-[#FECACA] bg-[#FEF2F2] p-4 space-y-2">
        ...
        <p className="text-[12px] text-[#B91C1C]" dir="auto">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="border border-[#FECACA] rounded px-2 py-1.5 text-sm font-mono flex-1"
            dir="ltr"
          />
          <button
            onClick={doReset}
            disabled={resetText !== RESET_PHRASE || resetting}
            className="flex items-center gap-1.5 bg-[#B91C1C] text-white text-sm font-semibold rounded-lg px-4 py-2 disabled:opacity-40"
          >
            {resetting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />} Reset to Fresh System
          </button>
        </div>
        {resetResult && (
          <div className="rounded-lg border border-[#BBF7D0] bg-[#F0FDF4] p-2.5 text-[12px] text-[#15803D] flex items-start gap-1.5">
