...
export default function TripFuelHistory({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-[#BFDBFE] bg-[#EFF6FF] px-3 py-2.5 text-[12px] text-[#1E3A8A] space-y-1.5" dir="auto">
        ...
        <p className="text-[#3730A3]">Click any row to see its full working. · تفصیل کے لیے قطار پر کلک کریں۔</p>
      </div>
                ...
                return (
                  <React.Fragment key={t.tripId}>
                    <tr
                      ...
                      className={`border-t border-[#F3F4F6] cursor-pointer hover:bg-[#F9FAFB] ${(t.variancePercent ?? 0) > 10 ? "bg-[#FEF2F2]" : ""}`}
                    >
                      ...
                      <td className="px-2 py-1.5 text-right tabular-nums text-[#B91C1C]">{t.fuelCost ? PKR(t.fuelCost) : "—"}</td>
                      <td className="px-2 py-1.5 text-right tabular-nums text-[#6B7280]">{t.benchmarkLitres ?? "—"}</td>
                      <td className={`px-2 py-1.5 text-right tabular-nums font-semibold ${(t.varianceLitres ?? 0) > 0 ? "text-[#B91C1C]" : "text-[#15803D]"}`}>
                      ...
                      <tr className="bg-[#F9FAFB]">
                        <td colSpan={13} className="px-4 py-3">
                          ...
                          {t.varianceLitres != null && t.variancePercent > 10 && (
                            <p className="mt-2 text-[11px] text-[#B91C1C]" dir="auto">
                          ...
                          {!t.litres && (
                            <p className="mt-2 text-[11px] text-[#B45309]" dir="auto">
...
function Tile({ label, value, tone }: { label: string; value: any; tone?: "good" | "bad" }) {
  const c = tone === "bad" ? "border-[#FECACA] bg-[#FEF2F2] text-[#B91C1C]" : tone === "good" ? "border-[#BBF7D0] bg-[#F0FDF4] text-[#15803D]" : "border-[#E5E7EB] bg-white text-[#1F2937]";
  return (
...
function Calc({ k, v, tone }: { k: string; v: React.ReactNode; tone?: "good" | "bad" }) {
  const c = tone === "bad" ? "text-[#B91C1C]" : tone === "good" ? "text-[#15803D]" : "text-slate-800";
  return (
